...
serve(async (req) => {
  ...
  try {
    ...
    const { imageBase64, secondImageBase64, styleCategory, styleSubcategory, photoType, gender, generationMode, refinementContext, makeupPreference, faceReferenceUrls, clientLocalMidnight, devMode, inspirationImageUrl, recreateMode, vibeLabel, viewAngle, customPrompt, frontViewImageUrl } = body;
    ...
    const stylePrompts: Record<string, { male: string; female: string }> = {
      ...
      swimwear: {
        female: "in a trendy Fashion Nova-style swimwear editorial, wearing a flattering coordinated two-piece bikini set — options include a halter bandeau top with high-waisted bottom, a triangle string bikini, an ombré bandeau set, or a cheeky Brazilian-cut bottom with matching top, in vibrant colors (sunset orange/pink ombré, tropical print, jewel tones, or chic solid black/white). Show as TWO separate matching pieces (no one-piece, no cover-up, no sarong, no kaftan). Styled with chunky gold hoop earrings, gold layered chains, and a slicked-back ponytail or sleek updo. Setting: luxury Mediterranean poolside, white-stucco Santorini wall, or marble pool deck with crystal water. Glossy, high-saturation Instagram fashion editorial photography with bright natural light, sharp focus, full-body shot showing the complete bikini fit and curves like a Fashion Nova product page.",
        male: "in a professional fashion editorial beach photoshoot, wearing tailored mid-length swim trunks in a bold print or solid color. CRITICAL: Do NOT add a shirt, towel, robe, or any fabric covering the chest or torso — the bare chest, shoulders, and stomach must be fully visible. Styled ONLY with leather slide sandals, aviator sunglasses, and a waterproof sport watch. Tropical beach resort setting with clear blue water. Professional fashion photography, editorial lighting.",
      },
      "urban-hiphop": {
    ...
    const swimwearSubStyleOverrides: Record<string, string> = {
      "beach-goddess": "IMPORTANT: Style as a luxurious one-piece swimsuit with plunging neckline OR a high-end matching bikini set. Add gold body chains, strappy gold sandals, and oversized sunglasses. NO sarong, wrap, or cover-up — leave skin fully visible. Goddess-like beach editorial aesthetic.",
      "sporty-swim": "IMPORTANT: Style as a sporty athletic swimsuit — racerback bikini top or high-neck crop top with matching boyshort or high-waist bottoms. Bold color blocking (neon, cobalt, coral). Sporty sandals and a visor cap. NO cover-up or wrap — skin must show. Active beachwear editorial.",
      "tropical-glam": "IMPORTANT: Style as a vibrant tropical-print two-piece bikini set — bold floral or palm print bikini top with matching bikini bottoms. Add platform espadrilles, shell jewelry, and a straw tote. NO sarong, kaftan, or wrap covering the body. Resort editorial style with skin visible.",
      "two-piece-set": "CRITICAL: This MUST be a clearly visible coordinated two-piece bikini set showing BOTH a separate bikini top AND separate matching bikini bottoms as distinct pieces. The bikini top and bikini bottom must both be clearly visible. Classic triangle or bandeau bikini top with matching high-cut or brazilian-cut bikini bottoms. Solid color or simple elegant pattern. Styled ONLY with sandals and sunglasses — NO sarong, wrap, cover-up, or fabric over the body. Bare midriff and skin must be visible. Fashion editorial beach photography.",
      "monokini": "IMPORTANT: Style as a designer cut-out one-piece swimsuit with strategic cutouts at the sides or waist, creating a modern sculpted silhouette. Bold solid color. Minimal accessories — just sunglasses and sandals. No cover-up. High-fashion editorial.",
      "swim-with-jewelry": "IMPORTANT: Style as a sleek solid-color two-piece bikini set accessorized with layered waterproof gold chains, body chains draped across the waist, anklets, stacking rings, and statement earrings. The jewelry is the focal point on bare skin. NO sarong or fabric cover-up. Beach editorial with golden-hour lighting.",
    };
    ...
    const isTeens = styleCategory === "teens";
    const isParentChild = styleCategory === "parent-child";
    const isCouples = styleCategory === "couples";
    const isChildCategory = isBaby || isKids;
    const isMinorCategory = isBaby || isKids || isTeens;
    const hasDualPhotos = (isParentChild || isCouples) && secondImageBase64;

    // Content safety for all minor categories (baby, kids, teens)
    const childSafetyDirective = isMinorCategory
      ? "\n\nCONTENT SAFETY — MANDATORY: This image involves a minor. ALL clothing MUST be age-appropriate, modest, and family-friendly. Do NOT generate anything revealing, suggestive, or inappropriate. No crop tops showing midriff, no low-cut items, no lingerie, no adult-themed styling. Keep the tone wholesome and suitable for a fashion catalog. Only generate fully-clothed, modest, and tasteful outfits."
      : "";
    ...
    const genderEnforcement = isBaby
      ...
      : isTeens
        ? `\n\nCRITICAL: This is a TEEN styling request (ages 13-17). The uploaded photo is of a teenager. Style this teen in trendy, age-appropriate clothing. The result must show a teenager — NOT an adult. All clothing must be appropriate for a teenager. Preserve the teen's EXACT face — same eyes, nose, mouth, skin tone, and hair.${childSafetyDirective}`
    ...
    if (isMannequin) {
      ...
      const mannequinType = isBaby
        ? `a baby-sized ${isMale ? "boy's" : "girl's"} mannequin`
        : isKids
      ...
      const hasFrontReference = isBackView && typeof frontViewImageUrl === "string" && frontViewImageUrl.length > 0;
      const backOutfitMatchNote = hasFrontReference
        ? ` AN ADDITIONAL REFERENCE IMAGE IS ATTACHED LAST showing the FRONT VIEW of the EXACT outfit this person is already wearing in the styled look. You MUST recreate that EXACT SAME outfit — every garment, every color, every fabric, every print, every accessory, every shoe, every piece of jewelry — identical to the front-view reference. Do NOT invent a new outfit. Do NOT change colors, fits, or any styling details. The ONLY difference between the front-view reference and your output is the camera angle (now showing the back instead of the front). Render how that exact outfit looks from behind — back of the top, back of the pants/skirt/dress, back of the shoes, back of the hair, back of any accessories.`
        : "";
      const poseAnglePreservation = isBackView
        ? ` BACK-VIEW SHOT REQUESTED: Generate this image as a REAR-VIEW photograph showing the person from behind. The camera is positioned BEHIND the person, capturing the back of their head, shoulders, back, and the back of the outfit. The person is facing AWAY from the camera. This is essential so the user can see how the outfit (especially swimwear, dresses, or back details) fits from behind. Keep the same overall pose energy, body shape, hair color/length (now seen from behind), and skin tone — but the entire viewpoint must be the rear of the body. Same setting and lighting as a natural editorial back-shot.${backOutfitMatchNote}`
      ...
      const keepNote = isSwimwear
        ...
        : isLingerie
          ? `${facePreservation} Keep the person's exact body shape and proportions.${poseAnglePreservation} Restyle ONLY their clothing to match the described luxury sleepwear and loungewear look. Change the background to an elegant bedroom or boudoir setting that matches the original camera angle. Professional fashion editorial style.`
          : isRevealing
            ? `${facePreservation} Keep the person's exact body shape and proportions.${poseAnglePreservation} Restyle ONLY their clothing to match the described outfit. Change the background to match the setting described while keeping the original camera angle. Professional fashion editorial style.`
            : `${facePreservation} Keep the person's exact body shape, proportions, and background.${poseAnglePreservation} Change ONLY their clothing and accessories. Realistic clothing, warm lighting.`;
      ...
      const inspirationNote = hasInspiration
        ? (recreateMode === "exact"
          ? `\n\nTWO IMAGES ARE ATTACHED — READ CAREFULLY:\n• IMAGE 1 (FIRST): The USER. This is the person who must appear in the final image. Preserve their exact face, head, hair, skin tone, body shape, proportions, height, and identity. The final image MUST be of THIS person — not the model in image 2.\n• IMAGE 2 (LAST): An OUTFIT REFERENCE${vibeLabel ? ` ("${vibeLabel}")` : ""} worn by a different model. IGNORE that model's face, body, hair, skin tone, and identity completely. ONLY copy the OUTFIT — every garment, color, fabric, print, fit, silhouette, layering, and visible accessories — and put it onto the USER from image 1.\n\nYour task: take the USER from image 1 and dress them in the EXACT outfit from image 2. Do NOT swap people. Do NOT keep the user's current clothes. Do NOT generate the model from image 2. The output must be the USER (image 1) wearing the OUTFIT (image 2), as if the user went and bought the same pieces and put them on.`
          : `\n\nTWO IMAGES ARE ATTACHED — READ CAREFULLY:\n• IMAGE 1 (FIRST): The USER — this person must appear in the final image. Preserve their exact identity, face, body, and proportions.\n• IMAGE 2 (LAST): A VIBE REFERENCE${vibeLabel ? ` for the "${vibeLabel}" aesthetic` : ""}. Use it ONLY as mood inspiration — color palette, silhouette family, overall energy. Do NOT copy it literally and do NOT generate the model in it. Create an ORIGINAL, authentic outfit tailored to the user that captures the same vibe.`)
        : "";
    ...
    if (!generatedImage) {
      const fallbackDescriptions: Record<string, string> = {
        swimwear: isMannequin
          ? `Resort fashion lookbook image of a ${isMale ? "male" : "female"} mannequin displaying a coordinated two-piece swimsuit — a structured swim top and matching swim bottoms in an elegant solid color, styled with strappy sandals and oversized sunglasses. No cover-up or wrap. Clean studio editorial image.`
          : `Restyle this ${genderWord} into a tasteful luxury resort editorial look. Dress them in a coordinated two-piece swimsuit (matching swim top and swim bottoms) suitable for a Vogue resort feature. No sarong, kaftan, or cover-up over the swimsuit. Add sandals and sunglasses as accessories. Keep the person's identity and body shape. Beautiful tropical beach resort background. Premium magazine photography with golden-hour lighting.`,
