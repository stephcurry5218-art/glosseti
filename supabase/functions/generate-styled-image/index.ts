    ...
    if (!generatedImage) {
      const fallbackDescriptions: Record<string, string> = {
        swimwear: isMannequin
          ? `Fashion lookbook image of a ${isMale ? "male" : "female"} mannequin displaying a coordinated two-piece bikini set — a structured bikini top and matching high-cut bikini bottoms in an elegant solid color, styled ONLY with strappy sandals and oversized sunglasses. NO sarong, wrap, or cover-up. Clean studio-quality editorial image.`
          : `Restyle this ${genderWord} into a luxury swimwear editorial look. Dress them in a flattering coordinated two-piece bikini set with a structured top and matching bottoms. CRITICAL: Do NOT add any sarong, pareo, wrap, kaftan, robe, towel, shirt, or fabric cover-up over the bikini — the bare skin (midriff, stomach, hips, shoulders) must be fully visible. Only add sandals and sunglasses as accessories. Keep the person's identity and body shape. Change background to a beautiful tropical beach resort. Premium magazine photography with golden-hour lighting.`,
