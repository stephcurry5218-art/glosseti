...
serve(async (req) => {
  ...
  try {
    ...
    const stylePrompts: Record<string, { male: string; female: string }> = {
      ...
      swimwear: {
        female: "in a tasteful luxury resort fashion editorial, wearing a coordinated two-piece swimsuit — a structured swim top with adjustable straps and matching swim bottoms in a rich solid color or elegant print, shown as two distinct coordinated pieces (no one-piece, no cover-up, no sarong, no wrap, no kaftan, no robe, no shirt layered over the swimsuit). Styled with strappy flat sandals, oversized designer sunglasses, and delicate gold layered jewelry. Beautiful tropical beach resort setting with turquoise water and palm trees. Professional Vogue-style resort editorial photography, golden-hour lighting, full body shot of the complete swimwear look.",
