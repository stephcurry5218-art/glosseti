        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14, marginBottom: 32 }}>
          {looks.map((look, i) => (
            <div key={look.name} className={`glamora-card anim-fadeUp d${i + 3}`} onClick={() => onLookSelect(look.name)}
              ...
              <div style={{ width: 52, height: 52, borderRadius: 16, background: "linear-gradient(135deg, hsla(12 52% 85% / 0.5), hsla(36 72% 88% / 0.5))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 26, flexShrink: 0 }}>{look.emoji}</div>
              <div style={{ flex: 1 }}>
              ...
              <div style={{ padding: "6px 12px", borderRadius: 100, background: "hsla(140 24% 58% / 0.15)", fontSize: 13, fontWeight: 600, color: "hsl(var(--glamora-success))" }}>{look.match}%</div>
            </div>
          ))}
        </div>
