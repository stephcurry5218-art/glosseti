...
const storeConfigs: Record<string, { base: (q: string) => string; affiliateParam?: string }> = {
  ...
  "Bloomingdale's": { base: (q) => `https://www.bloomingdales.com/shop/search?keyword=${encodeURIComponent(q)}` },
  "Bloomingdales": { base: (q) => `https://www.bloomingdales.com/shop/search?keyword=${encodeURIComponent(q)}` },
  "Saks Fifth Avenue": { base: (q) => `https://www.saksfifthavenue.com/search?q=${encodeURIComponent(q)}` },
  "Amazon Fashion": { base: (q) => `https://www.amazon.com/s?k=${encodeURIComponent(q)}&i=fashion`, affiliateParam: `tag=${AFFILIATE_TAG}` },
