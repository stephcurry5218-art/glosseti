...
import { Sparkles, Star, Camera, UserRound, Sun, CircleOff, ArrowRight, Info } from "lucide-react";
import type { UserPrefs, PhotoType } from "./GlamoraApp";

interface Props {
  prefs: UserPrefs;
  onBack: () => void;
  onGenerate: (iconName: string, file: File, photoType: PhotoType, base64: string) => void;
}
...
const InspirationScreen = ({ prefs, onBack, onGenerate }: Props) => {
  ...
  const canSubmit = iconName.trim().length >= 2 && file && base64;
      ...
      <div style={{ padding: "0 22px" }}>
        ...
        <div style={{ marginTop: 24 }}>
          <button
            ...
            onClick={() => canSubmit && onGenerate(iconName.trim(), file!, photoType, base64!)}
          >
            {canSubmit ? (<>Generate Inspired Look <Sparkles size={16} /></>) : "Enter an icon & upload a photo"}
          </button>
        </div>
