            ...
            {savedStyles.map((name, i) => {
              ...
              return (
                <div key={name} className={`glamora-card anim-fadeUp d${i + 1}`} onClick={() => onLookSelect(name)}
                  style={{ padding: "20px 18px", display: "flex", alignItems: "center", gap: 16, border: "1px solid hsla(36 50% 53% / 0.12)", cursor: "pointer" }}>
                  <div style={{ width: 56, height: 56, borderRadius: 18, background: "linear-gradient(135deg, hsla(12 52% 85% / 0.5), hsla(36 72% 88% / 0.5))", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 28, flexShrink: 0 }}>{meta.emoji}</div>
                  <div style={{ flex: 1 }}>
                    ...
                    <div style={{ fontSize: 11, color: "hsl(var(--glamora-success))", marginTop: 4, fontWeight: 600 }}>{meta.match}% match · Tap for full guide</div>
                  </div>
